...
#include "ekf_slam_pose_only/ekf_core.hpp"
...
public:
    EKFSlamNode() : Node("ekf_slam_pose_only_node"), last_odom_time_(0.0), prev_scan_pose_(Eigen::Vector4d::Zero()) {
        Eigen::Matrix4d process_noise   = Eigen::Matrix4d::Identity() * 0.01;
        Eigen::Matrix3d scanmatch_noise = Eigen::Matrix3d::Identity() * 0.5;

        ekf_ = std::make_unique<ekf_slam_pose_only::EKFCore>(process_noise, scanmatch_noise);
...
private:
    ...
    std::unique_ptr<ekf_slam_pose_only::EKFCore> ekf_;
