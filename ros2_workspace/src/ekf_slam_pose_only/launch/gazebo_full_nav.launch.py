...
HOVER_SCRIPT = os.path.join(
    PROJECT_DIR, 'ros2_workspace', 'src', 'ekf_slam_pose_only', 'scripts',
    'gz_hover.py')
BAG_DIR = os.path.join(PROJECT_DIR, 'results', 'gazebo_full_nav_ekf_pose_only_bag')
BAG_TOPICS = [
    '/crazyflie/odom',
    '/crazyflie/scan',
    '/ekf_pose',
...
def generate_launch_description():
    bridge_config = os.path.join(
        get_package_share_directory('ekf_slam_pose_only'), 'config', 'gz_bridge.yaml')
    ...
    ekf_slam = Node(
        package='ekf_slam_pose_only',
        executable='ekf_slam_pose_only_node',
        name='ekf_slam_pose_only_node',
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen')
