...
public:
    ...
    // Exposed as a static utility so callers (LandmarkFilter, node helpers)
    // can wrap angles without a separate header. Implementation unchanged
    // from v1 (still used internally).
